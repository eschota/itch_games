...
function normalizedFinite(value, fallback, min, max, decimals = 3) {
...
function normalizedMaterial(value, fallback) {
...
function normalizedFloodlight(value, fallback) {
    const source = recordSource(value);
    return {
...
export function normalizeVisualSettingsPatch(value, fallback = DEFAULT_VISUAL_SETTINGS) {
    const source = recordSource(value);
...
export function normalizeGameSettingsPatch(value, fallback = DEFAULT_GAME_SETTINGS) {
    ...
    for (const item of GAME_SETTINGS_SCHEMA) {
        ...
        if (item.input === "json") {
            if (item.key === "visual")
                result.visual = normalizeVisualSettingsPatch(raw, result.visual);
            continue;
        }
